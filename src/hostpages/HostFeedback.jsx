  ...
  useEffect(() => {
    const fetchData = async (user) => {
      try {
        ...
        const eventQuery = query(collection(db, "events"), where("hostId", "==", user.uid));
        ...
        const eventList = await Promise.all(
          eventSnapshot.docs.map(async (doc) => {
            ...
            // Fetch feedback count for the event
            const feedbackQuery = query(collection(db, "eventFeedback"), where("eventId", "==", doc.id));
            const feedbackSnap = await getDocs(feedbackQuery);
            const feedbackCount = feedbackSnap.size; // Count feedbacks
    
            return {
              id: doc.id,
              ...eventData,
              feedbackCount, // Set feedback count here
            };
          })
        );
